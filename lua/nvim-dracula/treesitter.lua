return {
  TSBoolean = { fg = C.bright_orange, bold = true },
  TSConstant = { fg = C.bright_cyan, italic = true },
  TSVariable = { fg = C.bright_purple },
  TSFunction = { fg = C.green, bold = true },
  TSKeyword = { fg = C.pink, bold = true },
  TSInclude = { fg = C.bright_purple, bold = true, italic = true },
  TSConstructor = { fg = C.cyan, bold = true },
  TSKeywordOperator = { fg = C.pink, bold = true },
  TSKeywordFunction = { fg = C.cyan, bold = true, italic = true },
  TSConditional = { fg = C.pink, italic = true, bold = true },
  TSRepeat = { fg = C.pink, italic = true, bold = true },
  TSField = { fg = C.fg },
  TSVariableBuiltin = { fg = C.bright_purple, italic = true },
  TSError = { fg = C.bright_red },
  TSPunctDelimiter = { fg = C.fg },
  TSPunctBracket = { fg = C.fg },
  TSPunctSpecial = { fg = C.fg },
  TSConstantBuiltin = { fg = C.bright_cyan },
  TSConstMacro = { fg = C.cyan },
  TSStringRegex = { fg = C.red },
  TSString = { fg = C.yellow },
  TSStringEscape = { fg = C.cyan },
  TSCharacter = { fg = C.green },
  TSNumber = { fg = C.purple },
  TSFloat = { fg = C.orange },
  TSAnnotation = { fg = C.yellow },
  TSAttribute = { fg = C.cyan },
  TSNamespace = { fg = C.bright_magenta },
  TSFuncBuiltin = { fg = C.cyan },
  TSFuncMacro = { link = "TSFunction" },
  TSParameter = { fg = C.orange },
  TSParameterReference = { fg = C.orange },
  TSMethod = { fg = C.green, bold = true },
  TSProperty = { fg = C.bright_purple },
  TSLabel = { fg = C.cyan },
  TSOperator = { fg = C.pink },
  TSException = { fg = C.purple },
  TSType = { fg = C.cyan, bold = true },
  TSTypeBuiltin = { fg = C.green },
  TSStructure = { fg = C.purple },
  TSText = { fg = C.fg },
  TSStrong = { fg = C.pink, bold = true }, -- bold
  TSEmphasis = { fg = C.green, italic = true }, -- italic
  TSUnderline = { fg = C.orange, underline = true },
  TSTitle = { fg = C.fg }, -- title
  TSLiteral = { fg = C.cyan }, -- inline code
  TSURI = { fg = C.bright_purple }, -- urls
  TSConstBuiltin = { fg = C.orange },
  TSStringEscap = { fg = C.yellow },
  TSTag = { fg = C.pink },
  TSTagDelimiter = { fg = C.fg },
  rainbowcol1 = { bold = true, fg = C.bright_magenta },
  rainbowcol2 = { bold = true, fg = C.bright_cyan },
  rainbowcol3 = { bold = true, fg = C.bright_purple },
  rainbowcol4 = { bold = true, fg = C.bright_green },
  rainbowcol5 = { bold = true, fg = C.bright_yellow },
  rainbowcol6 = { bold = true, fg = C.bright_magenta },
  rainbowcol7 = { bold = true, fg = C.bright_cyan },
}
